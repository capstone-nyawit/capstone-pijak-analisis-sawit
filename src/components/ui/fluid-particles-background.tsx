...
const COLOR_SCHEME = {
  // Hardcoding dark scheme for our agronomy theme
  dark: {
    particle: {
      color: "rgba(74, 222, 128, 0.07)", // emerald-400
    },
    background: "rgba(1, 23, 18, 0.12)", // #011712 with some opacity for trail
  },
} as const;
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      for (const particle of particles) {
        ...
        particle.velocity.x = Math.cos(angle) * 0.9;
        particle.velocity.y = Math.sin(angle) * 0.9;
